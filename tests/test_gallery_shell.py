...
class GalleryShellSmokeTestCase(unittest.TestCase):
    def test_gallery_page_hooks_exist(self) -> None:
        ...
        self.assertIn("礼物模型画廊", html)
        self.assertIn("公开礼物模型", html)
        self.assertIn("礼物模型列表", html)
        self.assertIn("公开可见", html)
        ...
        self.assertIn("work.html?demo=1", script)
        self.assertIn("暂时没有可展示的公开礼物模型", script)
